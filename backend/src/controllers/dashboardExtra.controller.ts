...
export const getHistoricalData = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Group by month/year
    const monthlyMap: Record<string, { month: number; year: number; omzet: number; hpp: number; transaksi: number; units: number }> = {};
    ...
    // Branch performance
    const branchMap: Record<string, { name: string; omzet: number; transaksi: number }> = {};
    transactions.forEach((tx: any) => {
...
export const getSalesSummary = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (branchId && branchId !== 'all') txWhere.branchId = String(branchId);
    ...
    // Cashier performance
    const cashierMap: Record<string, { name: string; omzet: number; transaksi: number }> = {};
    // Branch performance
    const branchPerfMap: Record<string, { name: string; omzet: number; transaksi: number; units: number }> = {};
    ...
    transactions.forEach((tx: any) => {
      ...
      if (!cashierMap[cid]) cashierMap[cid] = { name: tx.cashier?.name || 'Unknown', omzet: 0, transaksi: 0 };
      cashierMap[cid].omzet += tx.totalAmount;
      cashierMap[cid].transaksi += 1;

      // Branch
      const bid = tx.branchId;
      if (!branchPerfMap[bid]) branchPerfMap[bid] = { name: tx.branch?.name || bid, omzet: 0, transaksi: 0, units: 0 };
    ...
    const totalOmzet = transactions.reduce((s: number, t: any) => s + t.totalAmount, 0);
