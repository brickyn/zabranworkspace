...
export const createPartner = async (req: AuthRequest, res: Response) => {
  try {
    const partner = await prisma.b2BPartner.create({ data: req.body });
...
export const updatePartner = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const partner = await prisma.b2BPartner.update({ where: { id }, data: req.body });
...
export const deletePartner = async (req: AuthRequest, res: Response) => {
  try {
    const id = String(req.params.id);
    await prisma.b2BActivity.deleteMany({ where: { partnerId: id } });
    await prisma.b2BTransaction.deleteMany({ where: { partnerId: id } });
    await prisma.b2BMaintenanceLog.deleteMany({ where: { partnerId: id } });
...
export const getActivities = async (req: Request, res: Response) => {
  try {
    ...
    const activities = await prisma.b2BActivity.findMany({
      where,
...
export const createActivity = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.body.partnerId === 'NEW') {
      const newPartner = await prisma.b2BPartner.create({
        data: {
    ...
    if (req.body.scheduleId) {
      await prisma.b2BSchedule.update({
    ...
    if (req.body.result && req.body.result.startsWith('Deal') && finalPartnerId) {
      ...
      await prisma.b2BTransaction.create({
        data: {
...
export const getSchedules = async (req: Request, res: Response) => {
  try {
    const schedules = await prisma.b2BSchedule.findMany({
      include: {
...
export const createSchedule = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const schedule = await prisma.b2BSchedule.create({
      data: {
        partnerId: finalPartnerId || null,
        targetName: req.body.targetName,
...
export const createBatchSchedules = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const createdSchedules = await prisma.b2BSchedule.createMany({
...
export const getMaintenanceLogs = async (req: Request, res: Response) => {
  try {
    ...
    const logs = await prisma.b2BMaintenanceLog.findMany({
      where,
      include: { partner: { select: { name: true, company: true, category: true } } },
      orderBy: { date: 'desc' },
    });
...
export const createMaintenanceLog = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const log = await prisma.b2BMaintenanceLog.create({
      data: {
        partnerId: finalPartnerId,
...
export const getB2BMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const [totalPartners, activePartners, transactions, prevTransactions, newPartnersThisMonth, activities, prevActivities] = await Promise.all([
      ...
      prisma.b2BPartner.count({ where: { status: 'Active' } }),
      prisma.b2BTransaction.findMany({ where: { date: { gte: start, lte: end } } }),
      prisma.b2BTransaction.findMany({ where: { date: { gte: prevStart, lte: prevEnd } } }),
      prisma.b2BPartner.count({ where: { startDate: { gte: start, lte: end } } }),
      prisma.b2BActivity.count({ where: { date: { gte: start, lte: end } } }),
      prisma.b2BActivity.count({ where: { date: { gte: prevStart, lte: prevEnd } } })
    ]);
