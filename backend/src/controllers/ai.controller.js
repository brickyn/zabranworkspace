...
exports.aiChat = exports.analyzeCampaign = void 0;
...
const fetchCampaignDataForAI = async (adAccountId) => {
    ...
    const insightPromises = campaigns.map((camp) => fetch(`${getMetaBaseUrl()}/${camp.id}/insights?date_preset=maximum&fields=${fields}&access_token=${token}`)
        .then(r => r.json())
        .catch(() => null));
    ...
    const summary = campaigns.map((camp, i) => {
...
const analyzeCampaign = async (req, res) => {
    try {
        ...
        const txAggregate = await prisma_1.default.transaction.aggregate({
            _sum: { totalAmount: true },
            where: { status: 'Completed' }
        });
        const totalErpRevenue = txAggregate._sum.totalAmount || 0;
        const totalMetaSpend = campaignsData.reduce((acc, curr) => acc + curr.spend, 0);
...
exports.analyzeCampaign = analyzeCampaign;
